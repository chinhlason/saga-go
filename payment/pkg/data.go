...
func (r *Repository) Insert(ctx context.Context) error {
	_, err := r.db.ExecContext(ctx, "INSERT INTO payment (status) VALUES ($1)", "PENDING")
	return err
}

func (r *Repository) Get(ctx context.Context, id int) (*Order, error) {
	row := r.db.QueryRowContext(ctx, "SELECT * FROM payment WHERE id = ?", id)
	order := &Order{}
...
func (r *Repository) Update(ctx context.Context, id int, status string) error {
	_, err := r.db.ExecContext(ctx, "UPDATE payment SET status = ? WHERE id = ?", status, id)
	return err
}
